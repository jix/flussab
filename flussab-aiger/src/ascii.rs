...
use crate::{
    aig::{Aig, AndGate, Latch, OrderedAig, OrderedAndGate, OrderedLatch, Symbol, SymbolTarget},
...
impl Header {
    fn parse<L: Lit>(reader: &mut LineReader) -> Result<Self, ParseError> {
        token::fixed(reader, b"aag")
            .or_give_up(|| token::unexpected(reader, "ASCII AIGER header \"aag\""))?;
    ...
    L: Lit,
{
    ...
    pub fn inputs(self) -> Result<ParseInputs<'a, L>, ParseError> {
        Ok(ParseInputs {
            inputs_left: self.header.input_count,
            parser: self,
        })
    }
    ...
    pub fn parse(self) -> Result<Aig<L>, ParseError> {
        let mut aig = Aig {
            max_var_index: self.header.max_var_index,
            ..Aig::default()
        };

        aig.inputs.reserve(self.header.input_count);
        ...
        let mut aag_reader = self.inputs()?;
...
where
    L: Lit,
{
    pub fn next_latch(&mut self) -> Result<Option<Latch<L>>, ParseError> {
        if self.latches_left == 0 {
            return Ok(None);
        }
        self.latches_left -= 1;

        let state_code = token::lit(
            &mut self.parser.reader,
            "latch state literal",
            self.parser.max_lit,
            true,
        )?;
        let state = L::from_code(state_code);
        token::required_space(&mut self.parser.reader)?;
        ...
        if token::required_newline_or_space(&mut self.parser.reader)? {
            ...
            } else if initialization_code == state_code {
                initialization = None;
            } else {
...
where
    L: Lit,
{
    pub fn next_and_gate(&mut self) -> Result<Option<AndGate<L>>, ParseError> {
        if self.ands_left == 0 {
            return Ok(None);
        }
        self.ands_left -= 1;
        let output = L::from_code(token::lit(
            &mut self.parser.reader,
            "and gate output literal",
            self.parser.max_lit,
            true,
        )?);
        token::required_space(&mut self.parser.reader)?;
        let input_1 = L::from_code(token::lit(
            &mut self.parser.reader,
            "first and gate input literal",
            self.parser.max_lit,
            false,
        )?);
        token::required_space(&mut self.parser.reader)?;
        let input_2 = L::from_code(token::lit(
            &mut self.parser.reader,
            "second and gate input literal",
            self.parser.max_lit,
            false,
        )?);
        token::required_newline(&mut self.parser.reader)?;
...
// Using structural search replace command [L::code_from_lit($a, $b) ==>> $a.code()]

impl<'a, L> Writer<'a, L>
where
    L: Lit,
{
    pub fn new<'w>(writer: &'w mut DeferredWriter<'a>) -> &'w mut Self {
        unsafe { &mut *(writer as *mut DeferredWriter<'a> as *mut Self) }
    }
    ...
    pub fn write_header(&mut self, header: &Header) {
        ...
        self.writer.write_all_defer_err(b"aag");
    ...
    pub fn write_latch(&mut self, latch: Latch<L>) {
    ...
    pub fn write_and_gate(&mut self, and_gate: AndGate<L>) {
        flussab::write::text::ascii_digits(&mut self.writer, and_gate.output.code());
        self.writer.write_all_defer_err(b" ");
        flussab::write::text::ascii_digits(&mut self.writer, and_gate.inputs[0].code());
        self.writer.write_all_defer_err(b" ");
        flussab::write::text::ascii_digits(&mut self.writer, and_gate.inputs[1].code());
        self.writer.write_all_defer_err(b"\n");
    }
    ...
    pub fn write_aig(&mut self, aig: &Aig<L>) {
    ...
    pub fn write_ordered_aig(&mut self, aig: &OrderedAig<L>) {
        ...
        for _ in 0..aig.input_count {
            self.write_lit(L::from_code(code));
            code += 2;
        }
        ...
        for &OrderedAndGate { inputs } in &aig.and_gates {
            self.write_and_gate(AndGate {
                inputs,
                output: L::from_code(code),
            });
            code += 2;
        }
